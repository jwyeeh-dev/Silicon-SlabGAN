...
def do_supercell(image, n = None):
    ...
    image_x = make_representation.do_feature(atoms_x).reshape(1,1002,3)
    image_y = make_representation.do_feature(atoms_y).reshape(1,1002,3)
    image_z = make_representation.do_feature(atoms_z).reshape(1,1002,3)
    
    new_images = np.vstack((image_x,image_y,image_z))
    return new_images

if __name__ == "__main__":
    images = np.load('unique_slab_comp_dict',allow_pickle=True)
